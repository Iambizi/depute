...
```
  ...
  <Tab value="pnpm">
```bash
pnpm dlx ax-depute add orchestrator-view
```
  </Tab>
  <Tab value="yarn">
```bash
yarn dlx ax-depute add orchestrator-view
```
  </Tab>
  <Tab value="bun">
```bash
bunx ax-depute add orchestrator-view
```
  </Tab>
</Tabs>
...
While the preview above is static, here is the fully interactive component where you can collapse deeply nested trees, test node selection, and see depth-colors applied.

<StorybookEmbed story="ax-components-v1-orchestratorview--default" height={400} />
...
`OrchestratorView` acts as the navigation tree for a complex, hierarchical swarm.
- Use it as the left-pane side navigation in a specialized dashboard.
- Pair it with **`SwarmMonitor`** at the top of the viewport for aggregate metrics spanning the entire tree.
- When an individual node is clicked (`onNodeClick`), render a **`PlanCard`** + **`ToolTrace`** + **`RunControls`** trio in the main right-hand pane to inspect that specific sub-agent.
