...
The `onOpenItem` callback is designed to transition the user from the macro view (inbox) to the micro view (resolution). For `escalation`-type items, the standard pattern is to open an [**Escalation Router**](/docs/components/escalation-router) in a modal:
...
An inbox without actions is just a log. The `onOpenItem` callback transitions the user from the inbox to resolution — typically opening an [Escalation Router](/docs/components/escalation-router) or [Approval Gate](/docs/components/approval-gate) inside a modal.
