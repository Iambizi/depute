...
- **Automation Bias:** Users blindly rubber-stamp agent actions without scrutiny → *Fixed by `AutomationBiasAlert` and `ConfidenceMeter`*
- **Execution Amnesia:** Users lose track of the total "blast radius" of a massive agent session → *Fixed by `SessionOverview` and `StateDiff`*
- **Responsibility Diffusion:** Users approve actions assuming the machine is accountable → *Fixed by `DecisionRecord` and `BindingApproval`*
...
- *"I need a binding cryptographic signature for a ledger mutation"* → Use **`BindingApproval`** + **`TransactionReceipt`**
- *"I need to roll back a sequence of 40 API calls"* → Use **`RollbackTimeline`** + **`StateDiff`**
