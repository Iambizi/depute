...
- The agent proposes a plan → **PlanCard** shows it before execution
- The agent wants to send email → **ApprovalGate** blocks until the user approves
- The agent is running → **RunControls** let the user pause or stop at any time
- Multiple agents are coordinating → **OrchestratorView** shows who's managing whom
...
|------|-----------|---------|
| **v0** | 6 primitives | Single agent acting on behalf of a user |
| **v1** | 11 primitives | Multiple agents coordinating, delegating, or running in parallel |

Start with v0. Add v1 when you need orchestration.
