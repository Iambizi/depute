...
```
  ...
  <Tab value="pnpm">
```bash
pnpm dlx ax-depute@latest init
```
  </Tab>
  <Tab value="yarn">
```bash
yarn dlx ax-depute@latest init
```
  </Tab>
  <Tab value="bun">
```bash
bunx ax-depute@latest init
```
  </Tab>
</Tabs>

This command creates a `skills/depute/SKILL.md` entry in your repository. It instructs your agent on the exact composition flows and design rationale of all 18 Depute primitives, ensuring your agent builds reliable oversight UI on the first try.
