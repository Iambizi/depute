# AX Components: Production Setup Example

This example demonstrates how to replace the prototype's mock data simulators with an actual backend agent stream (e.g. from LangGraph, OpenAI Assistants API, or a custom SSE endpoint) while keeping the **exact same UI layer**.

## Concept

In production, your AX Components react declaratively to the state shape emitted by your server. 

The most common pattern is Server-Sent Events (SSE). As the server streams text chunks, JSON delta updates, or tool call events, your React hook (e.g., `useAgentStream`) accumulates them into the same arrays of `PlanStep` and `ToolCall` that our mock utilities output.
